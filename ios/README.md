...
- 로그인/회원가입 화면(`LoginView`)에서 **아이디/비번** 또는 **소셜**로 진입. 토큰은
  Keychain(`AuthService`)에 저장되고 소켓 핸드셰이크 `auth.token` 으로 전달된다.
- **채팅은 소셜(카카오/구글/애플) 연동 계정만** 가능(`user.chatEnabled`). 게임 화면 우하단
  💬 버튼 → `ChatView`.
- **소셜 SDK 설정(앱 단계):**
  - **Apple**: `SocialAuth` 에 시스템 프레임워크로 구현 완료. Xcode 에서 **"Sign in with
    Apple" Capability** 추가 + `APPLE_BUNDLE_ID`(서버) = 번들 ID 일치 필요.
  - **Google**: GoogleSignIn SPM 추가 후 `SocialAuth.token(.google)` 의 TODO 를 채워
    `idToken` 반환. 서버 `GOOGLE_CLIENT_IDS` 에 iOS 클라이언트 ID 등록.
  - **Kakao**: Kakao iOS SDK 추가 후 `SocialAuth.token(.kakao)` 의 TODO 를 채워 accessToken
    반환. `Info.plist` 에 URL Scheme(`kakao{앱키}`) 등록.

## TODO

- 보물/타일 아트 에셋 교체(현재 도형+번호 임시 렌더).
- 재연결(`lab:reconnect`) UI(앱 복귀 시 자동 복귀), 턴 타이머 카운트다운 표시.
